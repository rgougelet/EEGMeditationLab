...
for subjectId = {'104','105','107','210','211','212','213','103'} % add subject id's here
    for sessionId = {'0','1'}
        subjectsets = {};
...
%             EEG = eeg_checkset( EEG );
        ...
        if length(ALLEEG) == 3
            ...
            pop_saveset(ALLEEG(1), 'filename', ALLEEG(1).filename, 'filepath', 'E:\EEGMeditationLab\B_EqualNumChans');
            
            ALLEEG(2) = pop_select(ALLEEG(2),'channel',newchans);
            ALLEEG(2).event = []; ALLEEG(2).urevent = [];
            pop_saveset(ALLEEG(2), 'filename', ALLEEG(2).filename, 'filepath', 'E:\EEGMeditationLab\B_EqualNumChans');
            
            ALLEEG(3) = pop_select(ALLEEG(3),'channel',newchans);
            ALLEEG(3).event = []; ALLEEG(3).urevent = [];
            pop_saveset(ALLEEG(3), 'filename', ALLEEG(3).filename, 'filepath', 'E:\EEGMeditationLab\B_EqualNumChans');
